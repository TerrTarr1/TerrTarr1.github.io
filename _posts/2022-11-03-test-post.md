...
기호 이용한 강조
------------
어떠한 단어나 문장를 강조할 때, <span style="background-color: #d6aa9c"> ` </span> 이 기호 2개 사이에 적는다. 키보드에 보면 보통 물결표시( ~ )와 같이 있는 기호이다. 작성하면 아래처럼 나온다.<br>
`기호 이용한 강조 테스트`
...
```
색깔이 맘에 들지않으면, background-color: 에 정해놓은 색깔코드 넣으면 된다. 
